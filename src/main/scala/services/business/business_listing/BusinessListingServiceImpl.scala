...
import models.business.business_listing.{BusinessListing, BusinessListingCard}
...
class BusinessListingServiceImpl[F[_] : Concurrent : NonEmptyParallel : Monad](
  ...
  override def initiate(request: InitiateBusinessListingRequest): F[Option[BusinessListingCard]] = {
    ...
    for {
      createdListing: ValidatedNel[SqlErrors, Int] <- businessListingRepository.initiate(request)
